...
public class LCS_NOB {
  ...
  public LCS_NOB(int[] rows, int[] cols, int[] C) {
  ...
  private boolean backtrack(int[] cell, ArrayList<Integer> buffer) {
    ...
    int cval = cvalue(cell);

    int rval = rvalue(cell);
    ...
    if (value(up_cell) > value(left_cell)) {
